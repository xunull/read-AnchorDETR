...
class Transformer(nn.Module):
    def __init__(self,
                 # detr的d_model 默认是512,不过detr在main.py中的参数定义，默认也是256
                 d_model=256, nhead=8,
                 num_encoder_layers=6, num_decoder_layers=6,
                 # 这里与detr不同，detr的默认是2048
                 dim_feedforward=1024,
                 # 不用dropout，detr默认是0.1
                 dropout=0.,
                 activation="relu",
                 # 以下是detr没有的参数
                 # main.py中的参数默认值是1
        ...
        # 在仅使用一层特征层的情况下，encoder_layer_level无用
        ...
        # 也没有加1，与Deformable DETR相同
        num_classes = 91

        # 这两个在detr里面是不在transformer里面的，
    ...
    def forward(self, srcs, masks):
        ...
        if self.spatial_prior == "learned":
            # [300,2] -> [1,300,2] -> [bs,300*3,2]
            # [bs,300*3,2]
            # 第一个维度重复bs，第二个维度重复num_pattern
        ...
        # like (1,900,256) batch=1 level=1
        # [3,256] -> [1,3,1,256] -> [bs,3,300,256] -> [bs,3*300,256]
        ...
        if self.attention_type == "RCDA":
            # pos2posemb1d 三角函数高频编码，adapt_pos1d MLP
            # [bs,w,256]
        ...
        for lid, layer in enumerate(self.decoder_layers):
            # output 是decoder的输出，第一个decoder使用的output是pattern网络的参数值
            # reference_points 是参考点坐标，learned或者是固定的grid
            # posemb_row  posemb_col 是有效的坐标值（经过位置编码，mlp之后的）
            output = layer(output, reference_points, srcs, mask,
                           # 对 reference_points 使用
            ...
            # bbox的处理与Deformable DETR的处理相同，也是偏移量修正的意思
...
class TransformerEncoderLayerSpatial(nn.Module):
    ...
    def forward(self, src, padding_mask=None, posemb_row=None, posemb_col=None, posemb_2d=None):
        # self attention src [bz,256,h,w]
        # posemb_col [bz,w,256]
        # posemb_row [bs,h,256]

        bz, c, h, w = src.shape
        # [bs,h,w,c]
        src = src.permute(0, 2, 3, 1)  # [bz,w,h,256]

        if self.attention_type == "RCDA":
            posemb_row = posemb_row.unsqueeze(1).repeat(1, h, 1, 1)  # [bs,h,w,256]
            posemb_col = posemb_col.unsqueeze(2).repeat(1, 1, w, 1)  # [bs,h,w,256]
            src2 = self.self_attn((src + posemb_row).reshape(bz, h * w, c),
            ...
            # 标准的attention
            # 在进入到attention的各项的维度顺序为 [hw,bs,256]
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt,
                reference_points,
                srcs, src_padding_masks=None,
                adapt_pos2d=None,
                adapt_pos1d=None,
                posemb_row=None, posemb_col=None,
                posemb_2d=None):
        # tgt [bs,900,256]
        # srcs [1,1,256,h,w]
        # reference_points [1,900,2]
        # posemb_2d 已经在外面经过adapt_pos2d处理了

        tgt_len = tgt.shape[1]
        # [1,900,2] -> [1,900,256] -> [1,900,256]
        # pos2posemb2d 三角函数高频编码
        # 不管cross-attention是否使用了RCDA，但是self-attention依然是标准的Attention，那么就还是要使用标准形式的query_pos
        query_pos = adapt_pos2d(pos2posemb2d(reference_points))

        # self attention

        q = k = self.with_pos_embed(tgt, query_pos)
        # self attention还是标准的attention
        ...
        if self.attention_type == "RCDA":
            # [bs,900,256] pos2posemb1d 三角函数高频编码，adapt_pos1d MLP网络
            query_pos_x = adapt_pos1d(pos2posemb1d(reference_points[..., 0]))
            # [bs,900,256] pos2posemb1d 三角函数高频编码，adapt_pos1d MLP网络
            query_pos_y = adapt_pos1d(pos2posemb1d(reference_points[..., 1]))
            posemb_row = posemb_row.unsqueeze(1).repeat(1, h, 1, 1)  # [1,h,w,256]
            posemb_col = posemb_col.unsqueeze(2).repeat(1, 1, w, 1)  # [1,h,w,256]
            src_row = src_col = srcs
            k_row = src_row + posemb_row
            k_col = src_col + posemb_col
            tgt2 = \
                self.cross_attn((tgt + query_pos_x).repeat(l, 1, 1),
                                (tgt + query_pos_y).repeat(l, 1, 1),
                                k_row,
                                k_col,
                                srcs,
                                key_padding_mask=src_padding_masks)[0].transpose(0, 1)
        else:
            # [1,900,256]
...
def mask2pos(mask):
    # 这时True表示有效的地方
    not_mask = ~mask
    # 累计和，后面是False位置的，累计和不变，与最后一个True位置的相同
