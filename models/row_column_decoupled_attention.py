...
def multi_head_rcda_forward(query_row,  # type: Tensor
    ...
    """
    # batch size, tgt_len (如300个query，3个pattern就是900), 256
    bsz, tgt_len, embed_dim = query_row.size()
    src_len_row = key_row.size()[2]  # w
    src_len_col = key_col.size()[1]  # h
    ...
    ######################################## 进行query key value的proj ##################################################
    ########## for q_row
    # This is inline in_proj function with in_proj_weight and in_proj_bias
    # query的2个，key的2个，value的1个，都在一起 256*5 = 1280
    _b = in_proj_bias
    _start = 0
    _end = embed_dim
    ...
    q_col = q_col.transpose(0, 1)  # [hw,bs,256]
    k_row = k_row.mean(1).transpose(0, 1)  # [bz,h,w,256] -> mean(1) -> [bz,w,256] -> transpose -> [w,bz,256]
    k_col = k_col.mean(2).transpose(0, 1)  # [bz,h,w,256] -> mean(2) -> [bz,h,256] -> transpose -> [h,bz,256]

    q_row = q_row * scaling
    q_col = q_col * scaling

    q_row = q_row.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)  # [8,hw,32]
    q_col = q_col.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)  # [8,hw,32]

    if k_row is not None:
        k_row = k_row.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)  # [8,w,32]
    if k_col is not None:
        k_col = k_col.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)  # [8,h,32]
    if v is not None:
        # [8,h,w,32]
        v = v.contiguous().permute(1, 2, 0, 3).reshape(src_len_col, src_len_row, bsz * num_heads, head_dim).permute(2,
                                                                                                                    0,
                                                                                                                    1,
                                                                                                                    3)
    # q_row [8,hw,32] 和 k_row [8,w,32] 计算一次权重
    attn_output_weights_row = torch.bmm(q_row, k_row.transpose(1, 2))

    # q_col [8,hw,32] 和 k_col [8,h,32] 计算一次权重
    ...
    if efficient_compute:
        if src_len_col < src_len_row:
            ...
            # 与 v 进行计算
            # attn_output_weights_row is [8,hw,w]
            # v is [8,h,w,32] -> permute -> [8,w,h,32] -> reshape -> [8,w,h*32]
            # attn_output_weights_row matmul v ==> [8,hw,h*32] -> reshape -> [8,hw,h,32] -> permute -> [8,h,hw,32]
            attn_output_row = torch.matmul(attn_output_weights_row,
                                           v.permute(0, 2, 1, 3)
                                           .reshape(b_ein, w_ein, h_ein * c_ein)).reshape(b_ein, q_ein, h_ein,
                                                                                          c_ein).permute(0, 2, 1, 3)


            # attn_output_weights_col [8,hw,44] -> permute -> [hw,8,w] -> [hw,8,1,w]
            # attn_output_row [8,h,hw,32] -> permute -> [hw,8,w,32]
            # attn_output_weights_col matmul attn_output_row -> [hw,8,1,32] -> [hw,8,32] -> reshape -> [hw,bs,256]
            attn_output = torch.matmul(attn_output_weights_col.permute(1, 0, 2)[:, :, None, :],
                                       attn_output_row.permute(2, 0, 1, 3)).squeeze(-2).reshape(tgt_len, bsz, embed_dim)
            ...
            attn_output = torch.matmul(attn_output_weights_row[:, :, None, :], attn_output_col).squeeze(-2).permute(1,
                                                                                                                    0,
                                                                                                                    2).reshape(
        ...
        b_ein, q_ein, h_ein = attn_output_weights_col.shape
        b_ein, h_ein, w_ein, c_ein = v.shape
        attn_output_col = torch.matmul(attn_output_weights_col, v.reshape(b_ein, h_ein, w_ein * c_ein)).reshape(b_ein,
                                                                                                                q_ein,
                                                                                                                w_ein,
                                                                                                                c_ein)
        attn_output = torch.matmul(attn_output_weights_row[:, :, None, :], attn_output_col).squeeze(-2).permute(1, 0,
                                                                                                                2).reshape(
            tgt_len, bsz, embed_dim)
        ### the following code base on einsum get the same results
        # attn_output_col = torch.einsum("bqh,bhwc->bqwc", attn_output_weights_col, v)
    ...
    if need_weights:
        return attn_output, torch.einsum("bqw,bqh->qbhw", attn_output_weights_row, attn_output_weights_col).reshape(
...
class MultiheadRCDA(Module):
    ...
    __constants__ = ['q_row_proj_weight', 'q_col_proj_weight', 'k_row_proj_weight', 'k_col_proj_weight',
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.,
        ...
        if self._qkv_same_embed_dim is False:
            ...
            # 5个相同大小的，2个用于query (x y), 2个用于key (x y), 一个用于value
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_rcda_forward(
                ...
                attn_mask=attn_mask,
                # query与key和value的尺寸不相同，因此需要使用各自的proj_weight
