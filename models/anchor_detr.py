...
class AnchorDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_feature_levels, aux_loss=True):
        ...
        Parameters:
            ...
            num_feature_levels: 原始DETR这个地方的参数是num_queries
            aux_loss: True if auxiliary decoding losses (loss at each decoder layer) are to be used.
        """

        """
        class_embed bbox_embed query_embed 并没有在这进行定义，而是放到了transformer中进行了定义
        """
    ...
    def forward(self, samples: NestedTensor):
        ...
        # feature map 转换为transformer token
        # 与deformable detr相同，使用了多个特征层
        ...
        # 这个地方srcs cat了 masks没有cat
        srcs = torch.cat(srcs, dim=1)

        # 这个地方跟detr不一样，detr是先输出hs，后面还有class_embed和bbox_embed，才会得到class和coord的输出
        # 而anchor detr 将class_embed和bbox_embed放到了transformer中
